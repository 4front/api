...
var noDomainMatchRegex = [/No match for domain/i, /DOMAIN NOT FOUND/i];
...
module.exports = function(domainName, callback) {
  async.retry({times: 5, interval: 1000}, function(cb, results) {
    ...
    for (var i = 0; i < lines.length; i++) {
      if (noDomainMatchTest(lines[i])) return callback(null, null);
  ...
  function noDomainMatchTest(line) {
    return _.some(noDomainMatchRegex, function(re) {
      return re.test(line);
    });
  }
};
