...
module.exports = function() {
  ...
  router.get('/:env?', [hasRole('admin'), bodyParser.json(), validateEnvName], function(req, res, next) {
    if (req.params.env)
  ...
  router.put('/:key', [hasRole('admin'), bodyParser.json(), setGlobalEnvParam], put);

  // Set the value of an environment variable
  router.put('/:env/:key', [hasRole('admin'), bodyParser.json(), validateEnvName], put);

  // Delete global environment variable
  router.delete('/:key', [hasRole('admin'), bodyParser.json(), setGlobalEnvParam], del);

  // Delete a virtual environment specific environment variable
  router.delete('/:env/:key', [hasRole('admin'), bodyParser.json(), validateEnvName], del);
