...
module.exports = function(options) {
  ...
  router.post('/', bodyParser.json(), function(req, res, next) {
    ...
    if (!planInfo) {
      return next(Error.http(400, "Invalid plan name", {
        code: 'invalidPlanName', plan: orgData.plan
      }));
    }

    // Verify that the user has not already used up their trial
    if (orgData.plan === 'trial' && req.ext.user.usedTrialOrg === true) {
      return next(Error.http(400, "User already used up trial org", {
        code: 'userAlreadyUsedTrial'
      }));
    }
    ...
    async.parallel([
      ...
      if (orgData.plan === 'trial') {
        ...
        req.app.settings.membership.updateProfile(req.ext.user, function(_err) {
