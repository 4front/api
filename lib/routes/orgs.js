...
module.exports = function(options) {
  ...
  router.post('/', function(req, res, next) {
    ...
    var planInfo = options.orgPlans[orgData.plan];
    if (!planInfo)
    ...
    async.parallel([
      ...
      if (orgData.plan === 'trial') {
        ...
        options.database.updateUser(req.ext.user, function(err) {
          if (err) return next(err);
  ...
  router.put('/:orgId/terminate', hasRole('admin'), function(req, res, next) {
    ...
    async.parallel([
      function(cb) {
        options.database.updateOrganization({
          orgId: req.ext.organization.orgId,
          activated: false,
          terminated: true,
      ...
      function(cb) {
        options.database.deleteOrgMembers(req.ext.organization.orgId, cb);
      }
