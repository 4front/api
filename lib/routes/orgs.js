...
module.exports = function(options) {
  ...
  router.post('/', bodyParser.json(), function(req, res, next) {
    ...
    async.parallel([
      ...
      if (orgData.plan === 'trial') {
        ...
        req.app.settings.database.updateUser(req.ext.user, function(err) {
          if (err) return next(err);
  ...
  router.post('/:orgId/members', [bodyParser.json(), hasRole('admin')], function(req, res, next) {
    ...
    // Instead of an Aerobatic userId, we have the providerUserId.
    else {
      // First check to see if this user exists.
      debug("find user with providerUserId=%s", memberData.providerUserId);
      req.app.settings.database.findUser(memberData.providerUserId, memberData.provider, function(err, user) {
        if (err) return next(err);

        if (!user) {
          var userData = _.pick(memberData, 'providerUserId', 'provider', 'avatar', 'username');

          // Force the providerUserId and username to be lowercase
          _.extend(userData, {
            userId: shortid.generate(),
            username: userData.username.toLowerCase(),
            providerUserId: userData.providerUserId.toString()
          });

          req.app.settings.database.createUser(userData, function(err, user) {
            if (err) return next(err);
        ...
        // If the user already exists, just create the org member
          ...
          if (memberData.avatar) {
            req.app.settings.database.updateUser({userId: user.userId, avatar: memberData.avatar, username: memberData.username}, function(err) {
              if (err) return next(err);
              createMember(memberData);
            });
          }
