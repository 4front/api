...
var publicSuffixList = new PublicSuffixList();
publicSuffixList.initializeSync();
...
module.exports = function() {
  ...
  router.get('/', function(req, res, next) {
    async.waterfall([
      function(cb) {
        req.app.settings.database.listDomains(req.ext.organization.orgId, cb);
      },
      function(domains, cb) {
  ...
  router.delete('/', [ensure.domainManager, hasRole('admin')], function(req, res, next) {
    ...
    async.series([
      ...
      function(cb) {
        req.app.settings.database.deleteDomain(req.ext.organization.orgId, domainToDelete.domainName, cb);
      }
    ], function(err) {
      if (err) return next(err);
    ...
    async.series([
      function(cb) {
        req.app.settings.database.getDomain(domainName, function(err, domain) {
          ...
          // left off.
          if (domain && domain.orgId === req.ext.organization.orgId && !domain.cdnDistributionId) {
...
function validateDomainName(req, res, next) {
  ...
  if (publicSuffixList.validate(domainName) !== true || publicSuffixList.domain(domainName) !== domainName) {
