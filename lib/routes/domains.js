...
module.exports = function() {
  ...
  router.get('/', function(req, res, next) {
    ...
    async.series([
      function(cb) {
        db.listDomains(req.ext.organization.orgId, function(err, _domains) {
  ...
  router.post('/validate', function(req, res, next) {
    ...
    async.series([
      ...
      function(cb) {
        ...
        domainMgr.createCdnDistribution(domainName, domain.certificateId, function(err, _distro) {
          if (err) return cb(err);

          cdnDistribution = _distro;
          cb();
        });
      },
      function(cb) {
  ...
  router.get('/check', validateDomainName, function(req, res, next) {
    ...
    async.series([
      ...
      function(cb) {
        ...
        var existingCertId = domainCheck.existingDomain.certificateId;
        req.app.settings.domains.getCertificateStatus(existingCertId, function(err, status) {
          if (err) return cb(err);
  ...
  // Put the routes that require admin access below here.
  router.use([hasRole('admin'), ensure.domainManager]);
  ...
  router.delete('/legacy', function(req, res, next) {
    ...
    async.series([
      function(cb) {
        db.getLegacyDomain(req.body.domainName, function(err, _domain) {
          if (err) return cb(err);
          domain = _domain;
          cb();
        });
      ...
      function(cb) {
        ...
        domainMgr.unregisterLegacyDomain(domain.domain, domain.zone, cb);
      },
      function(cb) {
        db.deleteLegacyDomain(req.body.domainName, cb);
      }
    ], function(err) {
      if (err) return next(err);
      res.status(204).end();
  ...
  router.post('/request', validateDomainName, function(req, res, next) {
    var domainName = req.body.domainName;
    ...
    async.waterfall([
      ...
      function(cb) {
        cb();
        // TODO: Find any certificates for this domain that are in any
        // status but issued and delete them.
...
function ensureDomainExists(req, domainName, callback) {
  req.app.settings.database.getDomain(domainName, function(err, domain) {
    ...
    if (domain.orgId !== req.ext.organization.orgId) {
      return callback(Error.http(403, 'Cannot delete domain %s that ' +
        'does not belong to this org', domain.orgId));
    }
