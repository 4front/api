...
module.exports = function() {
  ...
  router.post('/request', [hasRole('admin'), validateDomainName, ensure.domainManager], function(req, res, next) {
    ...
    async.waterfall([
      ...
      function(cb) {
        cb();
        // TODO: Find any certificates for this domain that are in any status but issued and delete them.
      ...
      function(cb) {
        req.app.settings.domains.getCertificateStatus(domain.certificateId, function(err, status) {
          ...
          if (status === 'PENDING_VALIDATION') {
            return cb(Error.http(400, 'Certificate is still pending validation', {code: 'certNotApproved'}));
          } else if (status === 'VALIDATION_TIMED_OUT') {
            return cb(Error.http(400, 'Certificate is still pending validation', {code: 'validationTimedOut'}));
          } else if (status !== 'ISSUED') {
            return cb(Error.http(400, 'Certificate has status "' + status + '". It must have status of "ISSUED" to confirm.', {code: 'certNotApproved'}));
          }
          cb();
        });
      },
