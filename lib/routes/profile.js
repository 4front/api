...
module.exports = function(options) {
  ...
  router.post('/login', function(req, res, next) {
    var authProvider = req.app.settings.authProvider;
    authProvider.login(req.body.username, req.body.password, function(err, providerUser) {
      if (err) return next(err);

      req.app.settings.database.findUser(providerUser.userId, authProvider.name, function(err, user) {
        if (err) return next(err);

        // Generate a login token
        var expires = moment.utc().add('minutes', req.app.settings.jwtTokenExpireMinutes).valueOf();
