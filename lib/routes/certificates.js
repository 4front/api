...
module.exports = function() {
  ...
  router.delete('/', [ensure.domainManager, hasRole('admin')], function(req, res, next) {
    ...
    async.series([
      ...
      function(cb) {
        // Delete each domain associated with the certificate
        debug('delete %s domains associated with cert', certDomains.length);
        async.each(certDomains, function(domain, done) {
          domainHelper.deleteDomain(req, domain, done);
        }, cb);
      },
      function(cb) {
        req.app.settings.domains.deleteCertificate(certificate, cb);
      },
      function(cb) {
