    ...
    async.waterfall([
      ...
      function(certs, cb) {
        ...
        async.each(inProgressCerts, function(cert, nextCert) {
          ...
          req.app.settings.domains.getCertificateStatus(cert.name, function(err, status) {
            if (err) return nextCert(err);

            // If the status is no longer 'InProgress', update the value in the database.
            if (status !== 'InProgress') {
              cert.status = status;
              req.app.settings.database.updateCertificate({name: cert.name, status: status}, nextCert);
            } else {
              nextCert();
            }
          });
