...
module.exports = function(options) {
  ...
  router.head('/:appName', function(req, res, next) {
    options.appLookup({name: req.params.appName}, req.app.settings, function(err, app) {
  ...
  router.put('/:appId', [hasRole('admin'), validateApp], function(req, res, next) {
    ...
    // Check if the appName has changed. If so, ensure that it isn't already in use.
    // if (appData.req.virtualApp.name)

    options.database.updateApplication(appData, function(err, updatedApp) {
      if (err) return next(err);

      // Delete the application from application cache.
      // Reload the application and put it into cache.
      var lookupOptions = _.extend({}, req.app.settings, {
        forceReload: true
      });

      options.appLookup({id: appData.appId}, lookupOptions, function(err, virtualApp) {
        if (err) return next(err);
  ...
  router.delete('/:appId', hasRole('admin'), function(req, res, next) {
    ...
    async.parallel([
      ...
      function(cb) {
        options.storage.deleteObjects('aerobaticapp-versions', req.ext.virtualApp.appId, cb);
      }
    ], function(err) {
      if (err)
        return next(err);

      // options.appLookup.clear(req.ext.virtualApp);

      // options.cache.del('appname_' + req.virtualApp.name);
      // options.cache.del('app_' + req.virtualApp.appId);
      res.status(204).end();
    });
  });
