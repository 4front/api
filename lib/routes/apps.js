...
module.exports = function(options) {
  ...
  router.get('/:appId/env/:env?', [hasRole('admin'), bodyParser.json()], function(req, res, next) {
    var env = req.app.settings.virtualAppRegistry.env;
    if (req.params.env)
      res.json(env[req.params.env]);
    else
      res.json(env);
  });

  // Set the value of an environment variable
  router.put('/:appId/env/:env?/:key', [hasRole('admin'), bodyParser.json()], function(req, res, next) {
    ...
    if (req.params.env) {
      ...
      if (!_.contains(req.ext.organization.environments, req.params.env))
        return next(Error.http(400, "Invalid environment " + req.params.env, {code: 'invalidVirtualEnv'}));
    }
    ...
    var envOptions = {
      ...
      key: req.params.key,
      virtualEnv: req.params.env || '_default',
  ...
  router.delete('/:appId/env/:name', [hasRole('admin'), bodyParser.json()], function(req, res, next) {
