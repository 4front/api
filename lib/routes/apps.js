    ...
    async.parallel([
      function(callback) {
        async.each(newDomains, function(domainName, cb) {
          domainManager.register(domainName, function(err, zoneId) {
            req.app.settings.database.updateDomainZone(domainName, zoneId, cb);
          });
        }, callback);
      },
      function(callback) {
        unregisterDomains(req.app.settings, deletedDomains, callback);
      }
    ], function(err) {
      if (err) return next(err);
