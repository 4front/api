...
module.exports = function(options) {
  ...
  // Update application
  var updateAppMiddleware = [hasRole('admin'), validateApp];
  ...
  router.delete('/:appId', hasRole('admin'), function(req, res, next) {
    ...
    async.parallel([
      ...
      function(cb) {
        // Unregister domains
        disassociateFromDomains(req.app.settings, req.ext.virtualApp, cb);
      }
    ], function(err) {
      if (err) return next(err);
  ...
  function disassociateFromDomains(settings, virtualApp, callback) {
    if (_.isArray(virtualApp.domains) === false) return callback();

    // Update the domains setting the appId to null.
    async.each(virtualApp.domains, function(domainName, cb) {
      settings.database.updateDomain({
        domain: domainName,
        orgId: virtualApp.orgId,
        appId: null
      }, cb);
    }, callback);
  }
