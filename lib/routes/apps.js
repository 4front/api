...
module.exports = function(options) {
  ...
  // Create new app
  router.post('/', [hasRole('admin,contributor'),
    bodyParser.json(), validateApp], function(req, res, next) {
  ...
  // Update application
  router.put('/:appId', [hasRole('admin'), bodyParser.json(), validateApp],
    ...
    req.app.settings.database.updateApplication(appData, function(err) {
      ...
      req.app.settings.virtualAppRegistry.getById(
        appData.appId, {forceReload: true}, function(_err, virtualApp) {
  ...
  router.put('/:appId/domain', [hasRole('admin'), bodyParser.json()], function(req, res, next) {
    ...
    async.series([
      function(cb) {
        req.app.settings.domains.register(domainName, function(err, _zoneId) {
      ...
      req.app.settings.virtualAppRegistry.getById(
        req.ext.virtualApp.appId, {forceReload: true}, function(_err, virtualApp) {
        if (_err) return next(_err);
  ...
  router.delete('/:appId/domain', [hasRole('admin'), bodyParser.json()], function(req, res, next) {
    ...
    async.waterfall([
      function(cb) {
        req.app.settings.database.getDomain(domainName, function(err, domain) {
          ...
          if (domain.appId !== req.ext.virtualApp.appId) {
            return cb(Error.http(403, "Cannot delete domain " + domainName +
              " that does not belong to this app"));
          }
      ...
      function(cb) {
        req.app.settings.virtualAppRegistry.getById(
          req.ext.virtualApp.appId, {forceReload: true}, cb);
      }
    ], function(err, virtualApp) {
      if (err) return next(err);
  ...
  router.post('/:appId/traffic-rules/:env', [hasRole('admin'), bodyParser.json()],
    function(req, res, next) {
    req.app.settings.database.updateTrafficRules(req.ext.virtualApp.appId,
      req.params.env, req.body, function(err) {
      if (err) return next(err);
