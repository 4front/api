...
module.exports = function(options) {
  ...
  router.post('/upload/*', function(req, res, next) {
    ...
    var filePath = _.slice(pathParts, pathParts.indexOf('upload') + 1).join('/');

    debug('uploading file %s to dev sandbox', filePath);

    // Create a cacheKey consisting of the userId, appId, and the filePath
    var cacheKey = req.ext.virtualApp.appId  + "/" + req.ext.user.userId + "/" + filePath;
    ...
    //TODO: Store the SHA hash in a separate cache key.
    req.app.settings.cache.setex(cacheKey + '/sha', req.header('Content-Hash'), maxAge);
