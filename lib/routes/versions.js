...
module.exports = function() {
  ...
  router.put('/:versionId/complete', [hasRole('contributor,admin'), bodyParser.json()], function(req, res, next) {
    ...
    req.app.settings.deployer.versions.updateStatus(versionData, req.ext, req.body, function(err, version) {
      if (err) return next(err);
  ...
  router.get('/', function(req, res, next) {
    ...
    async.series([
      function(cb) {
        req.app.settings.database.listVersions(req.ext.virtualApp.appId, options, function(err, data) {
            ...
            _.each(trafficRules, function(ruleList, envName) {
              ...
              if (matchingRule) {
                version.trafficRules.push({
                  ...
                  url: req.app.settings.virtualAppRegistry.buildEnvUrl(req.ext.virtualApp, envName)
                });
              }
            });
          });
  ...
  router.post('/:versionId/deploy/*', hasRole('admin,contributor'), function(req, res, next) {
    ...
    req.app.settings.deployer.deploy(req.ext.virtualApp.appId, req.params.versionId, fileInfo, function(err) {
      if (err) return next(err);
  ...
  router.post('/:versionId/push/:envName', [hasRole('admin'), bodyParser.json()], function(req, res, next) {
    ...
    // Create rule forcing all traffic for the specified env to be directed to the specified version.
    asyncTasks.push(function(cb) {
