...
module.exports = function() {
  ...
  router.put('/:versionId/complete',
    [hasRole('contributor,admin'), bodyParser.json()],
    function(req, res, next) {
      ...
      var versions = req.app.settings.deployer.versions;
      versions.updateStatus(versionData, req.ext, req.body, function(err, version) {
        if (err) return next(err);
  ...
  router.get('/:versionId', function(req, res, next) {
    ...
    db.getVersion(req.ext.virtualApp.appId, req.params.versionId, function(err, version) {
      if (err) return next(err);

      if (!version) {
        return next(Error.http(404, 'Version ' + req.params.versionId + ' not found',
  ...
  router.get('/', function(req, res, next) {
    ...
    async.series([
      function(cb) {
        var db = req.app.settings.database;
        db.listVersions(req.ext.virtualApp.appId, options, function(err, data) {
  ...
  router.put('/:versionId',
    [hasRole('admin,contributor'), bodyParser.json()],
    function(req, res, next) {
  ...
  router.post('/:versionId/deploy/*', hasRole('admin,contributor'), function(req, res, next) {
    ...
    var deployer = req.app.settings.deployer;
    deployer.deploy(req.ext.virtualApp.appId, req.params.versionId, fileInfo, function(err) {
      if (err) return next(err);
  ...
  router.post('/:versionId/push/:envName',
    [hasRole('admin'), bodyParser.json()],
    function(req, res, next) {
      ...
      // Create rule forcing all traffic for the specified env to be
      // directed to the specified version.
      asyncTasks.push(function(cb) {
