...
module.exports = function(options) {
  ...
  router.put('/:versionId/complete', [hasRole('contributor,admin'), bodyParser.json()], function(req, res, next) {
    ...
    req.app.settings.deployer.updateVersionStatus(versionData, req.ext, req.body, function(err, version) {
      if (err) return next(err);
  ...
  router.get('/', function(req, res, next) {
    req.app.settings.database.listVersions(req.ext.virtualApp.appId, {limit: req.query.limit || 20}, function(err, versions) {
      if (err) return next(err);
