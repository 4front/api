...
module.exports = function(options) {
  return function(req, res, next, appId) {
    ...
    options.appLookup({id: appId}, lookupOptions, function(err, virtualApp) {
      ...
      if (virtualApp.orgId) {
        debug("ensure user %s is member of org %s", req.user.userId, virtualApp.orgId);
        options.database.getOrgMember(virtualApp.orgId, req.user.userId, function(err, orgMember) {
          if (err) return next(err);

          if (!orgMember) {
            debug("User %s is not a member of organization %s", req.user.userId, virtualApp.orgId);
          ...
          req.ext.orgRole = orgMember.role;
