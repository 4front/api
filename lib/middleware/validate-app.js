  ...
  return function(req, res, next) {
    ...
    if (!_.isEmpty(appData.domainName)) {
      if (_.isEmpty(appData.subDomain)) {
        return next(Error.http(400, 'Missing sub-domain', {code: 'missingSubDomain'}));
      }
    } else {
    ...
    if (!_.isEmpty(appData.domainName)) {
      ...
      asyncTasks.push(function(cb) {
        req.app.settings.database.getDomain(appData.domainName, function(err, domain) {
          if (err) return cb(err);
          if (!domain) {
      ...
      asyncTasks.push(function(cb) {
        ...
        req.app.settings.database.getAppIdByDomainName(appData.domainName, appData.subDomain, function(err, appId) {
          if (err) return cb(err);
          if (!appId) return cb();
