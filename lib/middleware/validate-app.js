...
module.exports = function(options) {
  return function (req, res, next) {
    ...
    if (_.isEmpty(appData.name) === true || /^[\w\-]{3,50}$/.test(appData.name) === false) {
      debug("app name is invalid");
      return next(Error.http(400, "App name " + appData.name + " is not valid", {code: 'invalidAppName'}));
    }

    // Check if this app name already exists.
    debug("Checking if appName is available: " + appData.name);
    options.database.getAppName(appData.name, function(err, appName) {
      if (err)
        return next(err);
